...
public class Tree23 <K extends Comparable<K>,V> implements Dizionario {
    ...
    public void insertHelp(Nodo root,K key,V value){
        Nodo padre=this.find(root,key,value);
        ...
        if(padre.is2Node()){//se ha solo 2 figli
            if (key.compareTo((K) root.getLeftKey()) <0){//nel caso vada come figlio sinistro
                ...
                padre.setCentralKey(padre.getLeftKey());//swappo la chiave snistra nella centrale
                padre.setCentralKey(key);//inserisco come chiave sinistra la chiave appena aggiunta
    ...
    public Nodo find(Nodo root, K key, V value){
        if(root.hasOnlyChildsLeafs()){
            return root;
        }else {
            if (key.compareTo((K) root.getLeftKey()) <0) {//se è minore della chiave sinistra
                return this.find(root.getLeftChild(),key,value);
                ...
                return this.find(root.getCentralChild(),key,value);
            } else  {
                return this.find(root.getRightChild(),key,value);
            }
        }
    }
}
